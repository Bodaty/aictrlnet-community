"""add_multi_tier_pattern_learning

Revision ID: 71644546b9dd
Revises: b2da379fcfc4
Create Date: 2025-10-28 00:27:30.333975

"""
from alembic import op
import sqlalchemy as sa


# revision identifiers, used by Alembic.
revision = '71644546b9dd'
down_revision = 'b2da379fcfc4'
branch_labels = None
depends_on = None


def upgrade() -> None:
    connection = op.get_bind()
    inspector = sa.inspect(connection)
    existing_tables = inspector.get_table_names()

    if 'learned_patterns' not in existing_tables:
        return

    existing_columns = [col['name'] for col in inspector.get_columns('learned_patterns')]
    columns_to_add = {
        'scope': sa.Column('scope', sa.String(length=20), nullable=False, server_default='user'),
        'user_id': sa.Column('user_id', sa.String(length=36), nullable=True),
        'organization_id': sa.Column('organization_id', sa.UUID(), nullable=True),
        'promoted_from_scope': sa.Column('promoted_from_scope', sa.String(length=20), nullable=True),
        'contributing_users_count': sa.Column('contributing_users_count', sa.Integer(), nullable=False, server_default='1'),
        'promotion_candidate': sa.Column('promotion_candidate', sa.Boolean(), nullable=False, server_default='false'),
        'is_shareable': sa.Column('is_shareable', sa.Boolean(), nullable=False, server_default='true'),
        'contains_sensitive_data': sa.Column('contains_sensitive_data', sa.Boolean(), nullable=False, server_default='false'),
        'anonymized': sa.Column('anonymized', sa.Boolean(), nullable=False, server_default='false'),
    }

    for col_name, col_def in columns_to_add.items():
        if col_name not in existing_columns:
            op.add_column('learned_patterns', col_def)

    existing_indexes = [idx['name'] for idx in inspector.get_indexes('learned_patterns')]
    if 'idx_pattern_promotion' not in existing_indexes:
        op.create_index('idx_pattern_promotion', 'learned_patterns', ['promotion_candidate', 'scope'], unique=False)
    if 'idx_pattern_scope' not in existing_indexes:
        op.create_index('idx_pattern_scope', 'learned_patterns', ['scope', 'user_id', 'organization_id'], unique=False)
    if 'ix_learned_patterns_organization_id' not in existing_indexes:
        op.create_index(op.f('ix_learned_patterns_organization_id'), 'learned_patterns', ['organization_id'], unique=False)
    if 'ix_learned_patterns_user_id' not in existing_indexes:
        op.create_index(op.f('ix_learned_patterns_user_id'), 'learned_patterns', ['user_id'], unique=False)

    existing_fks = [fk['constrained_columns'] for fk in inspector.get_foreign_keys('learned_patterns')]
    if ['user_id'] not in existing_fks:
        op.create_foreign_key(None, 'learned_patterns', 'users', ['user_id'], ['id'])


def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_constraint(None, 'learned_patterns', type_='foreignkey')
    op.drop_index(op.f('ix_learned_patterns_user_id'), table_name='learned_patterns')
    op.drop_index(op.f('ix_learned_patterns_organization_id'), table_name='learned_patterns')
    op.drop_index('idx_pattern_scope', table_name='learned_patterns')
    op.drop_index('idx_pattern_promotion', table_name='learned_patterns')
    op.drop_column('learned_patterns', 'anonymized')
    op.drop_column('learned_patterns', 'contains_sensitive_data')
    op.drop_column('learned_patterns', 'is_shareable')
    op.drop_column('learned_patterns', 'promotion_candidate')
    op.drop_column('learned_patterns', 'contributing_users_count')
    op.drop_column('learned_patterns', 'promoted_from_scope')
    op.drop_column('learned_patterns', 'organization_id')
    op.drop_column('learned_patterns', 'user_id')
    op.drop_column('learned_patterns', 'scope')
    # ### end Alembic commands ###